...
AS
BEGIN
    IF NOT EXISTS (SELECT name FROM [TestDB].sys.all_objects WHERE UPPER(name) = 'QUERY_STORE_AWR')
        BEGIN
            SELECT Txt.query_sql_text, pl.plan_id, qry.* INTO [TestDB].[dbo].[query_store_awr]  
            FROM sys.query_store_plan pl  
            INNER JOIN sys.query_store_query qry  
            ON pl.query_id = qry.query_id  
            INNER JOIN sys.query_store_query_text txt  
    ...
    ELSE
        BEGIN
            INSERT INTO [TestDB].[dbo].[query_store_awr]
            SELECT a.* 
            FROM (
                  SELECT txt.query_sql_text, pl.plan_id, qry.*  
                  FROM sys.query_store_plan pl  
            ...
            WHERE NOT EXISTS (
                              SELECT 1 
                              FROM [TestDB].[dbo].[query_store_awr] b
                              WHERE a.query_hash = b.query_hash
                              AND a.plan_id=b.plan_id
                              ) ;
        END;   
...
EXEC [MerchantradeMoneyUAT].[dbo].sp_query_store_awr;
